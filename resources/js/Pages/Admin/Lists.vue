  ...
  <AdminLayout title="Listes de diffusion">
    <div class="flex items-start gap-4 mb-6 p-4 bg-gradient-to-br from-violet-500/5 to-transparent border border-violet-500/15 rounded-2xl">
      ...
      <div class="flex-1">
        <p class="text-sm text-gray-700 dark:text-gray-200 leading-relaxed">
          ...
          <code class="px-1.5 py-0.5 bg-white dark:bg-zinc-900 rounded text-[11px] font-mono border border-gray-200 dark:border-zinc-700">team@</code>,
          <code class="px-1.5 py-0.5 bg-white dark:bg-zinc-900 rounded text-[11px] font-mono border border-gray-200 dark:border-zinc-700">support@</code>,
          <code class="px-1.5 py-0.5 bg-white dark:bg-zinc-900 rounded text-[11px] font-mono border border-gray-200 dark:border-zinc-700">annonces@</code>…
        </p>
    ...
    <form @submit.prevent="create" class="bg-white dark:bg-zinc-800 rounded-2xl border border-gray-200 dark:border-zinc-700 p-5 mb-6 shadow-sm">
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-2">
        <div class="md:col-span-4 flex items-center bg-gray-50 dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-700 focus-within:border-violet-500 focus-within:ring-2 focus-within:ring-violet-500/15 overflow-hidden">
        ...
        <input v-model="form.display_name" placeholder="Nom affiché (optionnel)"
               class="md:col-span-3 px-3 py-2.5 text-sm bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl focus:outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/15"/>
        <select v-model="form.scope" required
                class="md:col-span-3 px-3 py-2.5 text-sm bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl focus:outline-none focus:border-violet-500">
    ...
    <div v-if="lists.length" class="space-y-2">
      <Link v-for="l in lists" :key="l.id" :href="`/admin/lists/${l.id}`"
            class="block group bg-white dark:bg-zinc-800 rounded-2xl border border-gray-200 dark:border-zinc-700 p-4 hover:shadow-md hover:border-violet-500/30 transition">
