<template>
  <AdminLayout title="Comptes">
    <form @submit.prevent="create" class="grid grid-cols-1 md:grid-cols-6 gap-2 mb-6 bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-700 p-4">
      <input v-model="form.local" required placeholder="nom" class="md:col-span-2 px-3 py-2 border border-gray-300 dark:border-zinc-600 dark:bg-zinc-800 rounded-lg text-sm focus:outline-none focus:border-[#FF4D2E]"/>
      <select v-model="form.domain_id" required class="px-3 py-2 border border-gray-300 dark:border-zinc-600 dark:bg-zinc-800 rounded-lg text-sm">
        <option v-for="d in domains" :key="d.id" :value="d.id">@{{ d.name }}{{ d.is_primary ? ' ⭐' : '' }}</option>
      </select>
      <input v-model="form.password" type="password" required minlength="10" placeholder="Mot de passe (10+ car.)" class="px-3 py-2 border border-gray-300 dark:border-zinc-600 dark:bg-zinc-800 rounded-lg text-sm"/>
      <input v-model="form.display_name" placeholder="Nom affiché" class="px-3 py-2 border border-gray-300 dark:border-zinc-600 dark:bg-zinc-800 rounded-lg text-sm"/>
            ...
            <td class="px-4 py-2.5">
              <div class="flex items-center gap-1">
                <input type="number" min="10" :value="Math.round(a.quota_bytes/1024/1024)"
                       ...
                       class="w-20 px-2 py-1 text-sm border border-gray-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 rounded focus:outline-none focus:border-[#FF4D2E]" />
    ...
    <Teleport to="body">
      <div v-if="editing" class="admin-form fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" @click.self="closeEdit">
        <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-300 dark:border-zinc-600">
          <div class="px-5 py-4 border-b border-gray-200 dark:border-zinc-800 flex items-center justify-between sticky top-0 bg-white dark:bg-zinc-900 z-10">
            <h2 class="font-bold text-lg text-gray-900 dark:text-gray-100">✏️ Éditer le compte</h2>
            <button @click="closeEdit" class="w-8 h-8 rounded-md hover:bg-gray-100 dark:hover:bg-zinc-800 text-xl leading-none text-gray-500 dark:text-gray-400">×</button>
          </div>
          <form @submit.prevent="saveEdit" class="p-5 space-y-4">
            <!-- Email = local + domaine -->
            <div>
              <label class="block text-[10px] uppercase tracking-wider text-gray-500 dark:text-gray-400 font-semibold mb-1">Adresse email</label>
              <div class="flex items-center bg-white dark:bg-zinc-800 border border-gray-300 dark:border-zinc-600 rounded-lg overflow-hidden focus-within:border-[#FF4D2E] focus-within:ring-2 focus-within:ring-[#FF4D2E]/20">
                <input v-model="edit.local" required pattern="^[a-zA-Z0-9][a-zA-Z0-9._\-]{1,63}$"
                  class="flex-1 !bg-transparent !border-0 !ring-0 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:!ring-0"/>
                <span class="px-2 text-gray-500 dark:text-zinc-400 select-none">@</span>
                <select v-model="edit.domain_id" required class="!bg-transparent !border-0 !ring-0 pr-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none">
                  <option v-for="d in domains" :key="d.id" :value="d.id">{{ d.name }}{{ d.is_primary ? ' ⭐' : '' }}</option>
                </select>
              </div>
              <p v-if="errors.local" class="text-rose-500 text-xs mt-1">{{ errors.local }}</p>
              <p class="text-[11px] text-gray-500 dark:text-zinc-400 mt-1">⚠ Changer l'email renomme aussi la boîte mail Dovecot (le user devra reconfigurer ses clients IMAP/SMTP).</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                <label class="block text-[10px] uppercase tracking-wider text-gray-500 dark:text-gray-400 font-semibold mb-1">Quota (Mo)</label>
                <input v-model.number="edit.quota_mb" type="number" min="10" required
                  class="w-full px-3 py-2 text-sm bg-white dark:bg-zinc-800 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-zinc-600 rounded-lg focus:border-[#FF4D2E] focus:ring-2 focus:ring-[#FF4D2E]/20 focus:outline-none"/>
              </div>
            </div>

            <div class="flex items-center justify-end gap-2 pt-4 border-t border-gray-100 dark:border-zinc-800">
              <button type="button" @click="closeEdit" class="px-4 py-2 text-sm text-gray-600 dark:text-zinc-300 hover:text-gray-900 dark:hover:text-white">Annuler</button>
              <button type="submit" class="px-5 py-2 bg-[#FF4D2E] text-white rounded-lg text-sm font-bold hover:bg-[#df3c1f] shadow-sm">💾 Sauvegarder</button>
            </div>
          </form>
        </div>
      </div>
